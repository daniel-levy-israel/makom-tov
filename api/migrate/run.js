...
module.exports = async function handler(req, res) {
  ...
  const done = new Set((await db.query("SELECT source_url FROM image_migrations WHERE status='done'")).rows.map((row) => row.source_url));
  const failedRetryable = new Set((await db.query("SELECT source_url FROM image_migrations WHERE status='failed' AND attempts < 3")).rows.map((row) => row.source_url));
  const limit = Math.min(250, Math.max(1, Number(req.query.limit) || 160));
  const pending = sourceUrls.filter((url) => !done.has(url) && (!failedRetryable.size || failedRetryable.has(url) || !done.has(url))).slice(0, limit);
  ...
  return res.status(200).json({ total: sourceUrls.length, processed: pending.length, completed, remaining: sourceUrls.length - completed, counts });
};
