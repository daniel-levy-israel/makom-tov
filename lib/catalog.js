...
function getStaticCatalog() {
  if (!staticCatalog) {
    staticCatalog = JSON.parse(fs.readFileSync(path.join(process.cwd(), 'properties.json'), 'utf8'));
  }
...
function toApi(row) {
  return {
    ...
    googleReviews: row.google_reviews ?? 0, maps: row.maps || '', website: row.website || '',
